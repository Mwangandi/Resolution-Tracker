  ...
  async onApplicationBootstrap() {
    ...
    try {
      ...
      if (userCount === 0) {
        ...
        this.logger.log(`Skipping user seeding, database already has ${userCount} users`);
      }
      ...
      if (itemCount === 0) {
        ...
        await this.prisma.systemItem.createMany({
          data: [
            ...
            { id: 'st3', type: 'statusCategories', name: 'Assigned', meta: JSON.stringify({ badgeClass: 'bg-purple-100 text-purple-800' }) },
            { id: 'st4', type: 'statusCategories', name: 'In Progress', meta: JSON.stringify({ badgeClass: 'bg-orange-100 text-orange-800' }) },
            { id: 'st5', type: 'statusCategories', name: 'Pending Report Review', meta: JSON.stringify({ badgeClass: 'bg-blue-100 text-blue-800 border border-blue-200' }) },
            { id: 'st6', type: 'statusCategories', name: 'Done', meta: JSON.stringify({ badgeClass: 'bg-green-100 text-green-800' }) },
            { id: 'st7', type: 'statusCategories', name: 'Declined', meta: JSON.stringify({ badgeClass: 'bg-red-100 text-red-800' }) },
          ],
        });
      }
